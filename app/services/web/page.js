...
function toServiceCases(projects, serviceSlug, limit = 6) {
  return (projects ?? [])
    ...
    .map((p) => ({
      ...
      image: p.hero_image_url || "https://images.unsplash.com/photo-1460925895917-afdab827c52f?w=600&q=80",
      href: `/portfolio/${p.slug}`,
    }));
}

export async function generateMetadata() {
  const m = metaBySlug.web;
  return {
    title: m.title,
...
export default async function WebServicePage() {
  ...
  const cases = toServiceCases(projects, "web");
  return (
    <>
      <Header />
      <main>
        <article>
          ...
          <ServiceFeatureGrid items={d.features} title="Ключевые возможности" />
